...
def create_pkt(mac: str) -> bytes:
    ...
    msg.extend(struct.pack('B', checksum(msg[2:4]))) # 8 bit unsigned integer, msg[4]
    ...
    msg.extend(struct.pack('B', checksum(msg[7:15]))) # 8 bit unsigned integer, msg[15]
    msg.extend(mac_str_to_bytes(mac)) # 6 byte MAC address, msg[16:22]
    msg.extend(struct.pack('B', checksum(msg[16:22]))) # 8 bit unsigned integer, msg[28]
    return msg
